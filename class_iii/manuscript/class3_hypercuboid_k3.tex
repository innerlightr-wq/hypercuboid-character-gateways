...
\]
and at the level of surfaces the branch sextic of $X_{\rm III}$ is carried
\emph{onto} that of the $\lambda=-1$ member, with no residual scalar, by
the $\Q$-linear substitution $(x,t,w)\mapsto(x,\;t-x,\;-t-w)$. Hence
$X_{\rm III}$ is isomorphic over $\Q$ to $X_{-1}$. Both statements are
checked in the script
\texttt{class3\_aop\_identification.py}.

It is worth being exact about \emph{which} member this is.
\cite{AOP2002} evaluate $A(\lambda,q)$ in closed form through a quantity
of the shape $\chi(\lambda+1)\bigl(a(\lambda,q)^2-q\bigr)$, which requires
$\lambda+1$ to be invertible; van Geemen and Top likewise take
