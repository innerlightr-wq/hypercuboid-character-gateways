...
\thanks{ORCID: \href{https://orcid.org/0009-0007-0190-9143}{0009-0007-0190-9143}}
...
is how we came to it. This paper is a sequel to \emph{Finite-Field
Hypercuboid Character Sums and a Discriminant-8 Singular K3 Surface}
\cite{ClassesI_II}, which studied two of the three residual character-sum
classes, $\Sigma_I(p)$ and $\Sigma_{II}(p)$, arising from the
zero-diagonal complement collapse structure of a clean finite-field
hypercuboid residue system on four coordinates. That paper reduced
$\Sigma_I(p)$ to a two-variable sum realized, via an explicit singular K3
surface, in terms of the weight-$3$, level-$8$ newform
$f=\texttt{8.3.d.a}$, with CM by $\Q(\sqrt{-2})$ and transcendental
lattice $T(X)\cong\mathrm{diag}(2,4)$. The present paper resolves the
third class, $\Sigma_{III}(p)$, whose elementary vanishing for
$p\equiv3\pmod4$ was already established in \cite{ClassesI_II} but whose
value for $p\equiv1\pmod4$ was left as the genuine open problem of that
paper.
...
non-isomorphic \emph{over $\Q$} (Section~\ref{sec:AOP-precise} gives the
structural reason: their point counts already disagree at an explicit
witness prime, $p=7$), a fact compatible, once both surfaces' transcendental
lattices are identified as isomorphic to $\mathrm{diag}(2,2)$, with the
...
otherwise; over $\overline\Q$ this shared lattice does not by itself
distinguish them. We do not claim the modular form itself as new anywhere in
this paper.
...
contain the longer explicit local computations referenced throughout
the text.
...
underlying collapse mechanism is \cite{Collapse2026}, Theorem~3.1) the
...
the form associated to a \emph{pair}-type element, $L_{\{1,2\}}=x_0+x_1$
(the form fixed, up to sign, by the double transposition $(1\,3)(2\,4)\in
S_4$) -- a genuinely different orbit (the $(2,2)$ orbit above), with no
permutation relating Class III to Classes I or II.
...
legs are individually Galois-fixed. The lemma below shows that a tree
automorphism fixing all four legs is the identity: each leg is a leaf with
a unique neighbor, so fixing $L_1$ fixes $S_1$ and fixing $L_3$ fixes
$S_3$, and $S_2$ is the unique vertex adjacent to both. Hence Galois fixes
all seven components, and every component of the fiber is defined over
$\Q$ -- with no fiber resolved explicitly at any point in the argument.
...
torsion subgroup $\mathrm{MW}(X_{\rm III}/\Q(T))_{\rm tors}$, so
$[\NS(X_{\rm III}):\Triv(X_{\rm III})]=\#\mathrm{MW}_{\rm tors}$, and this
index is legitimate exactly because
Proposition~\ref{prop:torsion-complete} proves the torsion is \emph{all}
of $(\Z/2)^2$ -- not merely that it contains the three visible $2$-torsion
sections. Without that completeness the index, and hence the discriminant
below, would be only an upper bound on $|\disc\NS(X_{\rm III})|$, and
$T(X_{\rm III})$ would not be pinned down to a single isomorphism class.
...
=q-6q^5+9q^9+10q^{13}-30q^{17}+11q^{25}+\cdots,
\]
...
by level, and a search of the relevant space at $2$-power level shows $f$
is the unique such form of level $16$; its Nebentypus is $\chi_{-1}$, as
it must be for a weight-$3$, CM-by-$\Q(i)$ form (the Nebentypus of such a
form is the quadratic character of the CM field itself, ramified exactly
at the primes dividing the discriminant, here $2$). Note that $f$ is
singled out here on structural grounds -- weight, level, Nebentypus and CM
field -- \emph{before} any coefficient is compared with a character sum.
...
non-isomorphic over $\Q$, witnessed explicitly at $p=7$ rather than by a
numerical search over a range. \cite{AOP2002}'s evaluation reads
$A(\lambda,p)=\chi(\lambda+1)\bigl(a(\lambda,p)^2-p\bigr)$, and at
$\lambda=8$ the inner sum $a(8,p)$ is defined using $1/(\lambda+1)=1/9$,
which does not exist mod $3$; \emph{the prime $p=3$ is therefore not a
valid comparison prime for this formula at all}, not merely an
exceptional case of an otherwise-universal identity, and we do not use
it. At $p=7$, $\lambda+1=9\equiv2\pmod7$ is invertible, and since
$9=3^2$ is a nonzero square modulo every prime other than $3$,
$\chi(9,7)=1$. At this prime the inner sum vanishes, $a(8,7)=0$ (direct
computation: a sum of $7$ terms), giving
$A(8,7)=\chi(9,7)\bigl(0^2-7\bigr)=-7$, whereas
Proposition~\ref{prop:involution} gives $T(7)=0$ (an independent,
elementary computation, unrelated to \cite{AOP2002}'s formula). Since
$A(8,7)=-7\neq0=T(7)$, the two affine surfaces have different numbers of
$\mathbb{F}_7$-points. The prime $p=7$ is one of good reduction for $X_{\rm
III}$ (Section~\ref{sec:badprimes} establishes this at every odd prime)
and, since $7\nmid\lambda+1=9$, one at which \cite{AOP2002}'s formula for
$A(8,p)$ is defined and applies; hence $X_{\rm III}$ and
\cite{AOP2002}'s surface are not isomorphic over $\Q$, and a fortiori not
isomorphic as elliptic surfaces over $\Q(T)$. (The same vanishing
$a(8,p)=0$, and hence the same discrepancy $A(8,p)=-p\neq0=T(p)$, was
also checked directly at $p=11,19$; $p=7$ is used here as a convenient
single witness, not a distinguished one, and no step of this argument
uses or requires $p=3$.) Over $\overline\Q$ we make no such claim: both
are singular K3 surfaces with transcendental lattice of discriminant $4$,
which does not distinguish them. Possibilities (iii) and (vi) do not
...
The computations supporting this paper are in the repository
\url{https://github.com/innerlightr-wq/hypercuboid-character-gateways}
(directory \texttt{class\_iii/}), archived with the manuscript at
\texttt{doi:10.5281/zenodo.22713447}. All finite-field computations use
exact integer arithmetic: Legendre symbols via modular exponentiation,
never floating point. Reproducible items include the character sums
...
No external data were used; every quantity in this paper is computed from
the definitions. The scripts that reproduce the computational checks are
in the repository cited above and are archived with the manuscript on
Zenodo (\texttt{doi:10.5281/zenodo.22713447}). The manuscript source is
archived with the PDF. All results are reproducible from the scripts with
a standard Python installation and a computer algebra system for the
symbolic steps.

\subsection*{AI assistance}
Large language model tools were used during this work for exploratory
reasoning, for developing and reviewing the verification scripts, for
literature and provenance auditing, for checking proofs, and for
manuscript editing. No output of such a tool was treated as authoritative.
Every mathematical claim in this paper was verified independently of the
tool that suggested it: the algebraic identities by symbolic computation,
the fiber configuration and lattice data by direct calculation from the
Weierstrass model, the graph-automorphism count by exhaustive enumeration,
and the modular identification by expanding the eta product and comparing
with brute-force character-sum evaluations in exact integer arithmetic.
Bibliographic data were checked against the cited sources or their
registered metadata rather than against any model's recollection. The
...
independently verified by scripts in this repository's tracked
`class\_iii/scripts/' directory: the projectivization and
...
Both scripts, and the further falsification-battery scripts alongside
them, obtain the Hecke eigenvalues of $f=\eta^6(4z)=\texttt{16.3.c.a}$
from `newform\_16\_3\_c\_a.py', a self-contained module in the same
directory that computes them directly from the eta-product definition
above (no network access, no external file); one further script,
`class3\_round2\_falsification.py', additionally uses
`newform\_16\_3\_f\_a\_traces.py', which records a documented, dated
excerpt of the LMFDB's own trace data for the unrelated Galois orbit
\texttt{16.3.f.a} (used there only as a deliberate wrong-newform check),
since that orbit has no comparably elementary closed form.
...
\bibitem{Collapse2026}
E.~De Jes\'us, \emph{Zero-Diagonal Complement Collapse in Clean
Finite-Field Hypercuboid Residue Systems}, Zenodo (2026),
\url{https://doi.org/10.5281/zenodo.22216640}. Source of the
zero-diagonal complement collapse (its Theorem~3.1: on $D_{\rm full}=0$
the $2^n-2$ proper subset-sum conditions collapse to $2^{n-1}-1$
complement-pair conditions, and cleanness forces $p\equiv1\pmod4$), which
is the mechanism underlying the seven-form reduction recalled in
\S\ref{sec:sum}. Cited for attribution; \S\ref{sec:sum} restates
everything this paper uses, so no result here depends on consulting it.
